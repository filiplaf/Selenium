...
public class ProveraOmega {
    ...
    public static void casinoBalance() throws IOException, InterruptedException {
		...
		RestAssured.baseURI ="https://t-api.mozzartbet.com";
	    ...
	    Response res = given().
				...
				body(kredencijali).
				when().
				post("/MozzartWS/external.json/session-create").
	    ...
	    Response resp = given().
				...
				body(result).
				when().
				post("/MozzartWS/external.json/webuser-balance").
	  ...
	  //Iz JSON izdvajam casinoTotal i ispisujem ga u konzolu i JSON file
		...
		try(FileWriter file = new FileWriter("C:\\Git workspace\\Selenium\\WebAutomation\\src\\main\\java\\resources\\kazinoBalans.json")) 
		{
