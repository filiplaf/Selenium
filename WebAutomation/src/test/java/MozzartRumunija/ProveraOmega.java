...
public class ProveraOmega {
    ...
    public static void casinoBalance() throws IOException, InterruptedException {
		...
		RestAssured.baseURI ="http://t-api.mozzartbet.ro";
	    ...
	    Response res = given().
				...
				body(kredencijali).
				when().
				post("/MozzartWS-RO/external.json/session-create").
	    ...
	    Response resp = given().
				...
				body(result).
				when().
				post("/MozzartWS-RO/external.json/webuser-balance").
	  ...
	  //Iz JSON izdvajam casinoTotal i ispisujem ga u konzolu i JSON file
		...
		try(FileWriter file = new FileWriter("C:\\Git workspace\\Selenium\\WebAutomation\\src\\main\\java\\resources\\kazinoBalansRumunija.json")) 
		{
